...
template<class Iter>
typename iterator_traits<Iter>::value_type summ(Iter b, Iter e) {
    typename iterator_traits<Iter>::value_type ans = 0;
    for(Iter a = b; a != e; ++a)
        ans += *a;
    return  ans;
}
...
//    Polygon(V);
   ...
   V.push_back(Point(.5, 0));
   V.push_back(Point(0, .5*sqrt(3.)));
    
    Point   P[3] = {Point(-.5, 0), Point(.5, 0), Point(0, .5*sqrt(3.))};
cout << "summ = " << summ(P, P+3) << endl;
    
cout << areaSSS(1,1,1) << endl;
cout << areaSSA(1,1,PI/3) << endl;
cout << areaSAA(1,PI/3,PI/3) << endl;
cout << area(Point(-.5, 0), Point(.5, 0), Point(0, .5*sqrt(3.)) ) << endl;
cout << area(Point(1, 0), Point(0.5, .5*sqrt(3.)) ) << endl;
cout << Polygon(V).area() << endl;
cout << Polygon(P,P+3).area() << endl;

cout << dist(Line(Point(),Point(1,1)), Point(-1,1))<<endl;
cout << dist(Line(Point(),1), Point(-1,1))<<endl;
cout << dist(Line(1,Point()), Point(-1,1))<<endl;

cout << dist(Segment(Point(), Point(1, 0)), Point(3,2))<<endl;

cout<<sizeof(double)<<endl;

Line    ll = Line(Point(0,0),Point(1,1));
Point   pp = Point(1,2);

cout << mirror(ll, pp) << endl;

cout << dist(bisector(Point(0,0),Point(-1,1)), Point(1,1))<<endl;
